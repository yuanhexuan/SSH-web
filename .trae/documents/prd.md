...
WebSSH 是一个基于浏览器的 SSH 远程连接工具，提供图形化界面让用户通过网页直接连接远程服务器。支持密码认证和密钥认证两种方式，提供 HTTPS 转 SSH 和 SSH 直连两种连接模式，连接后提供完整的终端和文件管理图形化界面。
...
| 页面名称 | 模块名称 | 功能描述 |
|----------|----------|----------|
| 连接页面 | 连接配置表单 | 输入主机、端口、用户名，选择连接方式（HTTPS转SSH / SSH直连） |
...
用户打开网页 → 填写连接信息（主机/端口/用户名）→ 选择认证方式（密码/密钥）→ 选择连接模式（HTTPS转SSH / SSH直连）→ 点击连接 → 后端建立SSH通道 → 前端通过WebSocket接收终端数据 → 用户在图形化终端中操作 → 可切换到文件管理器进行文件操作
...
flowchart TD
    ...
    D --> F{"选择连接模式"}
    E --> F
    F -->|"HTTPS转SSH"| G["通过HTTPS WebSocket代理连接"]
